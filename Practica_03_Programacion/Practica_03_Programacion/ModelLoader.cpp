...
void ModelLoader::calcBoundaries(Vector3D vector) {
	...
	this->maxX = fmax(this->maxX, vector.GetCoordinateX());
	this->maxY = fmax(this->maxY, vector.GetCoordinateY());
	this->maxZ = fmax(this->maxZ, vector.GetCoordinateZ());
	this->minX = fmin(this->minX, vector.GetCoordinateX());
	this->minX = fmin(this->minY, vector.GetCoordinateY());
	this->minX = fmin(this->minZ, vector.GetCoordinateZ());
...
Triangle ModelLoader::center(Triangle triangle) {
	...
	Triangle centeredTriangle = Triangle(  triangle.GetVertex1() - modelCenter, 
		                                   triangle.GetVertex2() - modelCenter, 
		                                   triangle.GetVertex3() - modelCenter,
		                                   triangle.GetNormal1(),
		                                   triangle.GetNormal2(),
		                                   triangle.GetNormal3());
